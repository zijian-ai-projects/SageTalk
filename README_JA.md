...
### Codex グローバル skills に一括インストール

```bash
curl -fsSL https://raw.githubusercontent.com/zijian-ai-projects/SageTalk/main/scripts/install-codex-skills.sh | sh
```

インストール後に Codex を再起動してください。このコマンドは `sage-talk` とすべての `sage-*` leaf skills をまとめてインストールします。

### 方法 2: clone してローカルからインストール
